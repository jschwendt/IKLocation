//
//  WGLocation.h
//  WannaGo
//
//  Created by Andres on 5/22/14.
//  Copyright (c) 2014 Inaka Networks. All rights reserved.
//

#import <Foundation/Foundation.h>
#import <CoreLocation/CoreLocation.h>

@protocol WGLocationDelegate <NSObject>
- (void)wgManagerDidFailWithError:(NSError *)error;
- (void)wgManager:(id) manager didUpdateToLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation;
@end

@interface WGLocation : NSObject <CLLocationManagerDelegate>
@property (readonly) CLLocation* location;
@property (readonly) CLLocation* oldLocation;

@property (readonly) CGFloat latitude;
@property (readonly) CGFloat longitude;
@property (nonatomic, getter = isLocationAbailable) BOOL locationAvailable;

@property (nonatomic,readonly) NSString *city;
@property (nonatomic,readonly) NSString *state;
@property (nonatomic,readonly) NSString *country;

+ (instancetype)sharedLocation;
- (void) setDelegate:(id)delegate;
@end
